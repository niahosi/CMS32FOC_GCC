...
constexpr SpeedCounts rpm_to_speed_counts(Rpm rpm) noexcept
{
...
```cpp
auto speed = rpm_to_speed_counts<262144>(Rpm{60});
```
...
```cpp
constexpr uint16_t add_angle(uint16_t a, int16_t d) noexcept
{
...
2. 写 rpm_to_speed_counts()
3. 写 speed_counts_to_rpm()
...
```text
motor_control_current.c 里的 slew_s16/slew_s32
速度目标斜坡
iq 输出斜坡
```
...
static_assert(rpm_to_speed_counts<262144>(Rpm{60}).value == 262144,
...
template <int32_t CountsPerRev>
constexpr Rpm speed_counts_to_rpm(SpeedCounts speed) noexcept
{
    static_assert(CountsPerRev > 0, "CountsPerRev must be positive");
    return Rpm{
...
static_assert(MOT_SENSOR_POLE_PAIRS > 0U,
              "MOT_SENSOR_POLE_PAIRS must be non-zero");
static_assert((MOT_SENSOR_DIR == 1) || (MOT_SENSOR_DIR == -1),
              "MOT_SENSOR_DIR must be 1 or -1");
