...
> 当前最终落地形态见 `25-Cpp迁移-C_Cpp混编最小封装落地指南.md`。本文中早期出现的
> `g_motor_cmd`、`g_motor_watch`、`MotorControlCState` 属于迁移过程示例；当前主线
> 以 `g_mc_cmd` 命令入口和 `cms32::motor::g_motor` 状态入口为准。
...
constexpr int32_t to_speed(int16_t rpm)
{
...
constexpr SpeedCounts to_speed(Rpm rpm)
{
...
函数指针保存的是“函数入口地址”。它不是 C++ 的高级抽象，本质上是一个可以被调用的地址。
...
static void run_motor_task(void)
{
    MotorControl_RunSlowLoop();
}

static TaskFn task = run_motor_task;
```
...
当前工程已经有一种硬件层面的函数指针思想：启动文件里的中断向量表。`startup_CMS32M6510_gcc.S` 把 `ADC_IRQHandler`、`UART0_IRQHandler` 等函数地址放进 `.vectors`，硬件中断发生时按地址跳过去。
...
它不适合用来让快环“看起来更灵活”。FOC 快环里多一层间接跳转，会让单步、符号追踪和最坏执行时间更不直观。

### 适合使用的位置

第一类是串口命令分发表。

以后串口协议有多个命令时，可以不用写很长的 `switch`，而是用固定表：
...
using CommandHandler = void (*)(const DecodedCommand& command) noexcept;
...
void handle_set_speed(const DecodedCommand& command) noexcept
{
    g_motor_cmd.speed_ref_rpm = static_cast<int16_t>(command.value);
}

void handle_start_home(const DecodedCommand&) noexcept
{
    g_screw_axis_cmd.home_start_seq++;
}

constexpr CommandRoute kCommandRoutes[] = {
    {CommandId::SetSpeedRpm, handle_set_speed},
    {CommandId::StartHome, handle_start_home},
};
```
...
```cpp
bool dispatch_command(const DecodedCommand& command) noexcept
{
    for (const CommandRoute& route : kCommandRoutes)
    {
        if (route.id == command.id)
        {
...
这个适合，因为串口命令是低频入口，不在 20 kHz FOC 快环内。表是 `constexpr` 固定数组，不需要 `std::function`、堆分配或运行时注册。

第二类是应用层 ADC sample hook。

现在 `main.c` 在 ADC 中断里固定调用：
...
如果以后不同应用都想挂到“控制快环有效采样之后”，可以提供固定 hook：

```c
typedef void (*AppAdcHook)(void);

void App_SetAdcHook(AppAdcHook hook);
```

但当前只有 `ScrewAxis_OnAdcSample()` 一个使用者，直接调用更清楚，所以这不是马上要改的地方。

第三类是传感器后端。

如果以后编码器不只有 MA600，而是支持 MA600、AS5047、MT6701 或无感估算，可以设计一个固定驱动表：

```c
typedef struct
{
    uint8_t (*read_raw)(uint16_t* raw);
    uint8_t (*is_ready)(void);
} EncoderDriver;
```

当前只有 MA600，先保持直接函数调用。等第二种编码器真的出现，再引入这个表。

第四类是调试输出后端。

如果以后 watch 输出可以去 Ozone 全局变量、UART 帧、冻结诊断记录，可以定义：

```c
typedef void (*WatchSink)(const MotorControlWatch_t* watch);
```

这类输出不参与 FOC 数学内层，适合做可选后端。

第五类是状态机很大以后。

例如 `screw_axis.cpp` 当前用 `switch (state)` 很直观。只有当状态数量变多、每个状态处理很长时，才考虑函数表：

```cpp
using HomeStateFn = void (*)(void) noexcept;
```

现在不建议改，因为 `switch` 对 Ozone 单步和阅读更直接。

### 不适合使用的位置

这些位置优先保持直接调用或 `switch`：
...
SVPWM
电流采样重构内层
MotorControl 当前 VF / Speed / Current 快环分发
故障进入安全态路径
```

例如 `core.cpp` 里当前 mode 分发只有 VF、Speed、Current 三种，`switch` 比函数指针表更适合调试。

### 本项目规则

```text
低频命令、协议、驱动后端：可以用固定函数指针表
高频快环、FOC 数学、安全停机：不要用运行时函数指针分发
函数集合必须编译期固定，不做运行时注册表
不用 std::function
不用捕获 lambda 存进函数指针
函数指针目标必须 noexcept 或 C 函数
表项数量要小，查找成本要可估计
Ozone 需要观察的路径优先保留直接函数名
```

如果某个地方只是为了少写几行 `switch`，不要急着改函数指针。只有当“可替换后端”或者“命令路由表”成为真实需求时，函数指针才有价值。
...
```text
符号表只暴露 g_screw_axis_cmd/g_screw_axis_watch
回零状态机行为不变
Flash/RAM 没有异常增长
```
...
Board UART byte API
  ...
  -> Command decoder
  -> apply to g_mc_cmd / g_screw_axis_cmd
  -> response encoder
```
