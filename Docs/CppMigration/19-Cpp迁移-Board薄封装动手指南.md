...
UART policy 先暂缓。当前只有一个 UART0，串口通信阶段也先不做；现在写 `uart_policy.hpp` 只是把 `BoardConfig.h` 包一层，收益不够。
...
UART policy 适合以后有多个串口后端时再写：

```text
多个 UART 实例
同一套协议可切换 UART/USB CDC
不同板卡有不同串口配置
```

当前不写 `uart_policy.hpp`，也不把它放进编译检查。
...
arm-none-eabi-g++ -std=c++17 -mcpu=cortex-m0plus -mthumb \
  ...
  -I Firmware/MotorControl/FrozenCpp \
  -I Firmware/Support \
...
static_assert(cms32::board::clamp_pwm_duty(0U).value == PWM_DUTY_MIN,
              "PWM duty min clamp failed");
static_assert(cms32::board::clamp_pwm_duty(PWM_PERIOD).value == PWM_DUTY_MAX,
              "PWM duty max clamp failed");
static_assert(cms32::board::safe_center_duty().u.value == PWM_DUTY_50,
              "PWM safe center duty failed");
