...
> 当前最终落地形态见 `25-Cpp迁移-C_Cpp混编最小封装落地指南.md`。本文讲的是第四阶段
> 外壳迁移过程，文中的 `g_motor_cmd/g_motor_watch/MotorControlCState` 已被当前
> `g_mc_cmd` 命令入口和 `cms32::motor::g_motor` 状态入口取代。
...
也就是说，第四阶段开始时下面这些文件先继续保留，后续再逐步迁移：

```text
Firmware/MotorControl/LegacyC/motor_control_current.c
Firmware/MotorControl/LegacyC/motor_control_encoder.c
Firmware/MotorControl/LegacyC/motor_control_output.c
Firmware/MotorControl/LegacyC/motor_control_vf.c
Firmware/MotorControl/LegacyC/motor_control_watch.c
```
...
```text
types.hpp 和 command_sanitizer.hpp 属于 MotorControl，不属于 Support。
Support 只提供 clamp、units、irq_guard、enum_utils 这些通用工具。
```

### C++ 文件名为什么不用 motor_control 前缀

C 里需要 `motor_control_` 前缀，因为 C 没有 namespace。文件名、静态函数名、全局符号都靠前缀防止撞名。

C++ 这边已经有两层边界：

```text
目录边界：Firmware/MotorControl/Core/
命名空间：cms32::motor
```

所以 C++ 文件名继续写 `motor_control_config.hpp`、`motor_control_core.cpp` 反而是在重复信息。当前规则是：

```text
C ABI / C 文件 / 对外函数：
  保留 MotorControl_* 和 motor_control_*，保证链接、调试和外部接口稳定。

MotorControl/Cpp 内部文件：
  使用短名，例如 types.hpp、config.hpp、core.cpp、current.cpp。

跨模块通用工具：
  放到 Firmware/Support，名字描述工具本身，例如 units.hpp、clamp.hpp。
```
...
Firmware/MotorControl/Config/config.hpp
  等价于 MotorControl C++ config，不需要再把 motor_control 写进文件名。
```
...
```cpp
cms32::support::to_speed<EncoderConfig::counts_per_rev>(rpm);
```
...
public:
    MotorControlCommand_t sanitize(const volatile MotorControlCommand_t& input) const noexcept
    {
        ...
        {
            out.speed_ref =
                cms32::support::to_speed<MC_SPEED_COUNTS_PER_REV>(
...
```text
core.cpp include 了 irq_guard.hpp、enum_utils.hpp
command_sanitizer.hpp include 了 clamp.hpp、units.hpp
...
Firmware/Support/command_sanitizer.hpp
```
...
如果 `core.cpp` 里直接 include C 头，可能链接失败。
...
```text
1. 新建 Firmware/MotorControl/Core/
2. 写 types.hpp
3. 写 config.hpp
4. 写 command_sanitizer.hpp
5. 用临时 arm-none-eabi-g++ 命令检查三个头文件
6. 写 core.cpp
7. CMake 把 motor_control_c.c 换成 core.cpp
...
```text
include path 是否包含 Firmware/MotorControl/Core
cms32_support_cpp 是否链接进 cms32_motor_control
C 头是否放进 extern "C"
