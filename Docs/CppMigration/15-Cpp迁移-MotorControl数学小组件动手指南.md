...
> 当前最终落地形态见 `25-Cpp迁移-C_Cpp混编最小封装落地指南.md`。本文保留数学组件
> 教学；正式状态源已经从 `MotorControlCState` 演进为 `MotorController g_motor`。

它的顺序是：先独立练习并编译验证两个 header-only 小组件，再把验证过的组件接入 `current.cpp`：

```text
Firmware/MotorControl/Math/fixed_pi.hpp
Firmware/MotorControl/Math/speed_math.hpp
```

这两个文件第一版都应该是 header-only。写完先单独编译检查；通过后再替换 Current/Speed 快环里的速度数学和 PI 调用。

当前 `current.cpp` 还会复用 Support 层的 `slew_step()`：

```text
id_ref_active / iq_ref_active
  使用 slew_step<int16_t>()

speed_ref_active
  使用 slew_step<int32_t>()

speed_iq_ref
  保留 slew_speed_iq() 这个业务 helper，内部调用 slew_step<int16_t>()
```

这里不使用有状态 `SlewLimiter`，因为正式状态已经在 `MotorControlCState` 里。我们只复用算法，不复制状态。
...
C++ 版本不要继续照搬 `MotorControl_InternalSpeedCountsToRpm()` 这种 C 风格长前缀。C 需要用前缀表达模块边界，C++ 已经有 `cms32::motor` namespace，也可以用 `SpeedMath` 类型表达上下文。
...
SpeedMath::to_speed()
SpeedMath::ramp_step()
SpeedMath::in_deadband()
SpeedMath::error_rpm()
```

`to_rpm()` 这类名字单独看很短，但完整调用点是 `cms32::motor::SpeedMath::to_rpm(...)`，上下文已经足够清楚。它返回 `cms32::support::Rpm`，不是裸 `int16_t`。是否饱和限幅属于这个 MotorControl 速度数学组件的默认语义，不额外写进函数名。
...
struct SpeedMathConfig
{
    ...
    static constexpr int16_t command_deadband_rpm = SpeedLoopConfig::command_deadband_rpm;
};
...
struct SpeedMath
{
    ...
    static constexpr cms32::support::Rpm to_rpm(cms32::support::SpeedCounts speed) noexcept
    {
        const int32_t rpm =
    ...
    static constexpr cms32::support::SpeedCounts to_speed(
        cms32::support::Rpm rpm) noexcept
    {
        ...
        return cms32::support::SpeedCounts{cms32::support::clamp<int32_t>(
            speed, -ref_limit_counts(), ref_limit_counts())};
    }
...
static_assert(cms32::motor::SpeedMath::ramp_step() > 0, "bad ramp");

int main()
{
    const auto speed =
        cms32::motor::SpeedMath::to_speed(cms32::support::Rpm{100});
    return cms32::motor::SpeedMath::to_rpm(speed).value;
}
CPP
```
