...
/** @brief 运行时同步采样 U/V/W 三相，优先用于三电阻动态采样。 */
#define CURRENT_SAMPLE_UVW 4U
/** @brief 固定两相采样模式。 */
#define CURRENT_SAMPLE_MODE_FIXED_PAIR 0U
/** @brief 动态选择低边窗口最大的两相采样，第三相由 KCL 重构。 */
#define CURRENT_SAMPLE_MODE_DYNAMIC_PAIR 1U
/** @brief 三电阻优先采样：三相共同窗口足够时采三相，否则退回动态两相。 */
#define CURRENT_SAMPLE_MODE_DYNAMIC_3SHUNT 2U
/** @brief VW 优先动态两相：默认 VW@650，窗口不足时才退到 UW/UV。 */
#define CURRENT_SAMPLE_MODE_VW_PREFERRED_DYNAMIC_PAIR 3U
/** @brief 运行时采样模式，按已验证经验优先在下降沿约 650 tick 采 VW。 */
#define CURRENT_SAMPLE_MODE CURRENT_SAMPLE_MODE_VW_PREFERRED_DYNAMIC_PAIR
/** @brief 固定邻域双点采样开关，当前关闭，保持 650 tick 单点采样。 */
#define CURRENT_SAMPLE_MULTI_ENABLE 0U
/** @brief 动态选择采样窗口。VW 优先动态两相需要打开。 */
#define CURRENT_SAMPLE_DYNAMIC_ENABLE 1U
/** @brief 已实测稳定的 VW 固定触发 tick。 */
#define CURRENT_SAMPLE_VW_TRIGGER_TICK 650U
...
/** @brief VW 从 fallback 回切时需要多出的窗口余量，避免临界抖动。 */
#define CURRENT_SAMPLE_SWITCH_HYST_TICK 64U
/** @brief pair 切换后的最小保持 PWM 周期数。 */
#define CURRENT_SAMPLE_PAIR_MIN_HOLD_PWM 8U
/** @brief 相邻采样跳变超过该 count 记一次尖峰。 */
#define CURRENT_SAMPLE_SPIKE_LIMIT_CNT 100
