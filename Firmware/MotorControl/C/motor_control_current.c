...
void MotorControl_CurrentRunFastLoop(MotorControlCState *mc, uint8_t speed_mode)
{
    ...
    else
    {
        ...
        iq_ref = mc->current_command.iq_ref;
    }

    mc->id_ref_active = slew_s16(mc->id_ref_active, mc->current_command.id_ref, CTRL_CUR_REF_RAMP_STEP);
    mc->iq_ref_active =
        slew_s16(mc->iq_ref_active,
                 foc_clamp_s16(iq_ref, (int16_t)-mc->current_command.iq_limit, mc->current_command.iq_limit),
     ...
     */
    theta_used = (uint16_t)(mc->encoder_elec + (uint16_t)mc->current_command.voltage_theta_offset);
    ...
    foc_pi_set_gains(&mc->current_pi_d,
                     mc->current_command.current_kp,
                     mc->current_command.current_ki,
                     (int16_t)-v_limit,
                     v_limit,
                     CTRL_CUR_PI_SHIFT);
    foc_pi_set_gains(&mc->current_pi_q,
                     mc->current_command.current_kp,
                     mc->current_command.current_ki,
                     (int16_t)-v_limit,
                     v_limit,
    ...
    mc->voltage_dq.d = foc_pi_update(&mc->current_pi_d, mc->id_ref_active, mc->current_dq.d);
    mc->voltage_dq.q = foc_pi_update(&mc->current_pi_q, mc->iq_ref_active, mc->current_dq.q);
    MotorControl_InternalApplyVoltageVector(mc, mc->voltage_dq.d, mc->voltage_dq.q, theta_used);
    mc->fast_loop_count++;
}
...
static void update_speed_loop(MotorControlCState *mc)
{
    const int32_t ref_target = mc->speed_command.speed_ref;
    ...
    int16_t iq_min = (int16_t)-mc->speed_command.iq_limit;
    int16_t iq_max = mc->speed_command.iq_limit;
    int16_t ref_rpm;

    mc->speed_loop_count++;
    mc->speed_ref_active = slew_s32(mc->speed_ref_active, ref_target, speed_ref_ramp_step_counts());
    ref_rpm = MotorControl_InternalSpeedCountsToRpm(mc->speed_ref_active);
    mc->speed_err_rpm = (int16_t)foc_clamp_s32((int32_t)ref_rpm - (int32_t)fb_rpm, -32768, 32767);
    ...
    foc_pi_set_gains(&mc->speed_pi,
                     mc->speed_command.speed_kp,
                     mc->speed_command.speed_ki,
                     iq_min,
                     iq_max,
                     CTRL_SPD_ERR_SHIFT);
