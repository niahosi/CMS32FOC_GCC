...
public:
    ...
    snapshot(const volatile MotorControlCommand_t& input) const noexcept
    {
        ...
        out.position_ref = input.position_ref;
        out.position_kp = input.position_kp;
        out.position_err_shift = input.position_err_shift;
        out.position_speed_limit_rpm = input.position_speed_limit_rpm;
        out.position_deadband_counts = input.position_deadband_counts;
    ...
    {
        command.iq_limit =
            positive_limit(command.iq_limit, CurrentLoopConfig::ref_limit);
        ...
        command.position_kp = clamp_gain(command.position_kp);
        command.position_err_shift =
            cms32::support::clamp<uint8_t>(command.position_err_shift, 0U, 30U);
        command.position_speed_limit_rpm =
            positive_limit(command.position_speed_limit_rpm,
                           static_cast<int16_t>(SpeedLoopConfig::ref_limit_rpm));
        command.position_deadband_counts =
            cms32::support::clamp<int32_t>(command.position_deadband_counts,
                                           0,
                                           EncoderConfig::counts_per_rev);
        command.current_v_limit =
            positive_limit(command.current_v_limit, PwmConfig::svpwm_voltage_limit);
        command.id_ref = symmetric_limit(command.id_ref, CurrentLoopConfig::ref_limit);
        command.iq_ref = symmetric_limit(command.iq_ref, command.iq_limit);
        command.open_loop_speed_ref =
            cms32::support::clamp<int32_t>(command.open_loop_speed_ref,
                                           -SpeedLoopConfig::ref_limit_counts,
                                           SpeedLoopConfig::ref_limit_counts);

        if (command.speed_ref_rpm != 0)
        {
            command.speed_ref = cms32::support::to_speed<EncoderConfig::counts_per_rev>(
                                    cms32::support::Rpm{command.speed_ref_rpm})
                                    .value;
        }

        command.speed_ref =
            cms32::support::clamp<int32_t>(command.speed_ref,
                                           -SpeedLoopConfig::ref_limit_counts,
                                           SpeedLoopConfig::ref_limit_counts);
        command.vf_voltage =
            symmetric_limit(command.vf_voltage, CurrentLoopConfig::voltage_limit);

        return command;
    }

    MCCurrentCommand_t
    current_command(const MotorControlCommand_t& command) const noexcept
    {
    ...
    MCPositionCommand_t
    position_command(const MotorControlCommand_t& command) const noexcept
    {
        return MCPositionCommand_t{command.position_ref,
                                   command.position_kp,
                                   command.position_err_shift,
                                   command.position_speed_limit_rpm,
                                   command.position_deadband_counts};
...
private:
    ...
    {
        return cms32::support::clamp<int16_t>(value,
    ...
    static constexpr int16_t positive_limit(int16_t value, int16_t limit) noexcept
    {
        ...
        return static_cast<int16_t>(
            cms32::support::clamp<int32_t>(magnitude, 0, limit));
    }
};
} // namespace cms32::motor
