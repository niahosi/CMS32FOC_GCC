 ...
 * Firmware/MotorControl/Core/encoder.cpp and split g_mc_* state objects.
 */
...
{
    mc->closed_loop.encoder.raw = 0U;
    mc->closed_loop.encoder.elec = 0U;
    mc->closed_loop.encoder.prev_raw = 0U;
    mc->closed_loop.encoder.delta = 0;
    mc->closed_loop.encoder.pos = 0;
    g_motor_diag.encoder_raw_step = 0;
    g_motor_diag.encoder_reject_step = 0;
    g_motor_diag.encoder_reject_prev_raw = 0U;
    g_motor_diag.encoder_reject_raw = 0U;
    g_motor_diag.encoder_reject_count = 0U;
    g_motor_diag.encoder_retry_count = 0U;
    g_motor_diag.encoder_retry_accept_count = 0U;
    g_motor_diag.encoder_retry_raw = 0U;
    g_motor_diag.encoder_hold_count = 0U;
    g_motor_diag.speed_reject_count = 0U;
    g_motor_diag.speed_reject_delta = 0;
    mc->closed_loop.speed.feedback = 0;
    mc->closed_loop.speed.feedback_filter = 0;
    ...
    mc->closed_loop.speed.startup_blank = CTRL_SPD_STARTUP_BLANK_SAMPLES;
    mc->closed_loop.encoder.age = 255U;
    mc->closed_loop.encoder.ok = 0U;
    mc->closed_loop.encoder.initialized = 0U;
}
...
#if MOT_SENSOR_DIR > 0
    return (uint16_t)(zero + (int32_t)raw);
#else
    return (uint16_t)(zero - (int32_t)raw);
#endif
}
...
static int16_t encoder_raw_delta(MotorControlCState *mc, uint16_t raw)
{
    return (int16_t)(raw - mc->closed_loop.encoder.raw);
}
...
static uint8_t encoder_raw_plausible(MotorControlCState *mc, uint16_t raw)
{
    ...
    if (mc->closed_loop.encoder.initialized == 0U)
    {
...
static uint8_t hold_last_encoder_angle(MotorControlCState *mc)
{
    g_motor_diag.encoder_hold_count++;
    if (mc->closed_loop.encoder.age < 255U)
    {
        mc->closed_loop.encoder.age++;
    }

    mc->closed_loop.encoder.ok =
        (uint8_t)((mc->closed_loop.encoder.initialized != 0U) &&
                  (mc->closed_loop.encoder.age <= MOT_ANGLE_MAX_AGE));
    return mc->closed_loop.encoder.ok;
}

/** @brief 记录坏角样本并保持上一角度。 */
static uint8_t reject_bad_encoder_angle(MotorControlCState *mc, uint16_t raw)
{
    g_motor_diag.encoder_reject_count++;
    g_motor_diag.encoder_reject_step = encoder_raw_delta(mc, raw);
    g_motor_diag.encoder_reject_prev_raw = mc->closed_loop.encoder.raw;
    g_motor_diag.encoder_reject_raw = raw;
...
static void accept_encoder_angle(MotorControlCState *mc, uint16_t raw)
{
    ...
    {
        g_motor_diag.encoder_raw_step = 0;
        mc->closed_loop.encoder.prev_raw = raw;
        mc->closed_loop.encoder.delta = 0;
        mc->closed_loop.encoder.initialized = 1U;
    }
    else
    {
        g_motor_diag.encoder_raw_step = encoder_raw_delta(mc, raw);
    }

    mc->closed_loop.encoder.raw = raw;
    mc->closed_loop.encoder.elec =
        electrical_from_raw(raw, mc->command.current.elec_zero_trim);
    mc->closed_loop.encoder.ok = 1U;
    mc->closed_loop.encoder.age = 0U;
}
...
static uint8_t retry_encoder_angle(MotorControlCState *mc)
{
    ...
    g_motor_diag.encoder_retry_count++;
    ...
    g_motor_diag.encoder_retry_raw = retry_raw;
    if (encoder_raw_plausible(mc, retry_raw) == 0U)
    {
        return 0U;
    }

    g_motor_diag.encoder_retry_accept_count++;
...
static uint8_t update_encoder_speed_state(MotorControlCState *mc)
{
    ...
    raw = mc->closed_loop.encoder.raw;
    if (mc->closed_loop.encoder.initialized == 0U)
    {
        mc->closed_loop.encoder.prev_raw = raw;
        mc->closed_loop.encoder.delta = 0;
    ...
    delta = (int16_t)(raw - mc->closed_loop.encoder.prev_raw);
    ...
    if ((delta > (int16_t)max_delta) || (delta < -(int16_t)max_delta))
    {
         ...
         */
        g_motor_diag.speed_reject_count++;
        g_motor_diag.speed_reject_delta = delta;
        mc->closed_loop.encoder.prev_raw = raw;
        mc->closed_loop.encoder.delta = 0;
        mc->closed_loop.speed.feedback_filter = 0;
        return 1U;
    ...
    mc->closed_loop.encoder.prev_raw = raw;
    mc->closed_loop.encoder.delta = delta;
    mc->closed_loop.encoder.pos += delta;

    if (mc->closed_loop.speed.startup_blank != 0U)
    {
        /* 模式切入后的前几次速度样本只建立 prev/raw 连续性，不输出速度。 */
        mc->closed_loop.speed.startup_blank--;
        mc->closed_loop.speed.feedback = 0;
        mc->closed_loop.speed.feedback_filter = 0;
        return 1U;
    }
    ...
    mc->closed_loop.speed.feedback_filter +=
        (speed_sample - mc->closed_loop.speed.feedback_filter) >> CTRL_SPD_FILTER_SHIFT;
    mc->closed_loop.speed.feedback = mc->closed_loop.speed.feedback_filter;

    if ((mc->closed_loop.speed.feedback > -CTRL_SPD_ZERO_SNAP) &&
        (mc->closed_loop.speed.feedback < CTRL_SPD_ZERO_SNAP))
    {
        mc->closed_loop.speed.feedback = 0;
    }
    return 1U;
