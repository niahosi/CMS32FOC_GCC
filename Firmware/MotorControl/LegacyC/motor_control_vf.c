 ...
 * Firmware/MotorControl/Core/vf.cpp and split g_mc_* state objects.
 */
...
void MotorControlVf_RunFastLoop(MotorControlCState *mc)
{
    ...
    s_vf.open_loop_timeout_ticks = (uint32_t)mc->command.vf.open_loop_timeout_ms * 2U;
    if ((s_vf.open_loop_timeout_ticks > 0U) &&
        (s_vf.open_loop_ticks >= s_vf.open_loop_timeout_ticks))
    {
        mc->runtime.state = MC_STATE_FAULT;
        mc->runtime.fault = MC_FAULT_OPEN_LOOP_TIMEOUT;
        MotorControl_InternalEnterSafeState(mc);
        return;
    }

    theta = update_open_loop_theta(&mc->command.vf);
    MotorControl_InternalApplyVoltageVector(mc, 0, mc->command.vf.vf_voltage, theta);
