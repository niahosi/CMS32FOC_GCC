...
void ADC_ConfigRunMode(uint32_t RunMode, uint32_t ConvertMode, uint32_t ClkDiv, uint32_t Holdtime)
{
    ...
    ADC->CON &=
        ~(ADC_CON_ADMODE10_Msk | ADC_CON_ADCMS_Msk | ADC_CON_ADCDIV_Msk | ADC_CON_ADCNSMP_Msk);
...
{
    ADC->LOCK = ADC_LOCK_WRITE_KEY;
    ADC->CON2 |= ADC_CON2_ADCSMPWAIT_Msk;
    ADC->LOCK = 0x00;
}
/*****************************************************************************
...
*****************************************************************************/
void ADC_ConfigCompare(uint8_t ACMPx, uint32_t CmpMode, uint32_t CmpCount, uint32_t CmpChannel,
                       uint32_t CmpData)
{
    ADC->LOCK = ADC_LOCK_WRITE_KEY;
    if (ACMPx == ADC_CMP0)
    {
        ADC->CMP0 &= ~(ADC_CMP_ADCCMPCOND_Msk | ADC_CMP_ADCCMPMCNT_Msk | ADC_CMP_ADCCMPCHS_Msk |
                       ADC_CMP_ADCCMPDATA_Msk);
        ADC->CMP0 |= CmpMode | ((0xf & CmpCount) << ADC_CMP_ADCCMPMCNT_Pos) |
                     ((0x1f & CmpChannel) << ADC_CMP_ADCCMPCHS_Pos) | (0xfff & CmpData);
    }
    else
    {
        ADC->CMP1 &= ~(ADC_CMP_ADCCMPCOND_Msk | ADC_CMP_ADCCMPMCNT_Msk | ADC_CMP_ADCCMPCHS_Msk |
                       ADC_CMP_ADCCMPDATA_Msk);
        ADC->CMP1 |= CmpMode | ((0xf & CmpCount) << ADC_CMP_ADCCMPMCNT_Pos) |
                     ((0x1f & CmpChannel) << ADC_CMP_ADCCMPCHS_Pos) | (0xfff & CmpData);
    }
    ADC->LOCK = 0x00;
}
